...
class connman:
    ...
    def load_values(self):
        try:
            ...
            if os.path.isfile(regFile):
                ...
                regCode = "(" + regLine[-2:] + ")"
                regValue = next((v for v in regList if regCode in v), "NONE")
            else:
                regValue = "NONE"
    ...
    def custom_regdom(self, **kwargs):
        try:
            ...
            self.oe.set_busy(1)
            if 'listItem' in kwargs:
                if str((kwargs['listItem']).getProperty('value')) == "NONE":
